...
import PythonWorker from 'worker-loader?filename=worker1.[fullhash].js!workers/python-worker';
import PythonTestWorker from 'worker-loader?filename=worker2.[fullhash].js!workers/python-test-worker';
...
export function getPythonTestWorker(): Worker {
    if (!testWorker) {
        testWorker = new PythonTestWorker();
    }
    return testWorker;
}
