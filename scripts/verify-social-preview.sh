...
Usage:
  YEAR=2025 POST_ID=abc123 ./scripts/verify-social-preview.sh

Biến môi trường (tùy chọn):
  FRONTEND_BASE   Mặc định: https://spacedev.vn
...
if printf '%s\n' "$FRONT_OG" | grep -qiF "$DEFAULT_TITLE"; then
    ...
    echo "      → Kiểm tra include deploy/nginx-social-preview*.conf và reload nginx."
fi
